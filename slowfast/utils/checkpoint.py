...
def _add_encoder_model_aliases(pre_train_dict, model_dict):
    """
    Support checkpoints saved before the DINOv3 HF wrapper introduced an
    extra ``encoder.model`` module level.
    """
    remapped = OrderedDict(pre_train_dict)
    added = 0
    for key, value in pre_train_dict.items():
        if not key.startswith("encoder.") or key.startswith("encoder.model."):
            continue
        alias = "encoder.model." + key[len("encoder.") :]
        if (
            alias in model_dict
            and alias not in remapped
            and value.size() == model_dict[alias].size()
        ):
            remapped[alias] = value
            added += 1
    if added:
        logger.info(
            "Added %d DINOv3 encoder checkpoint compatibility aliases "
...
def load_checkpoint(
    ...
    if convert_from_caffe2:
        ...
        if inflation:
            ...
            pre_train_dict = _add_encoder_model_aliases(
                checkpoint["model_state"], model_dict
            )
